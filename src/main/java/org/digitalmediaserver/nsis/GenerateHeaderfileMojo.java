...
/**
 * Generate a <code>project.nsh</code> containing NSIS variables that represent
 * the common POM values.
 * <p>
 * The resulting <code>target/project.nsh</code> (configurable) can then be
 * loaded into the main <code>setup.nsi</code> script via the include command.
 * </p>
 *
 * <pre>
...
public class GenerateHeaderfileMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		try {
			...
			writer.println("; Generated by {0} from pom.xml version {1}", System.getProperty("user.name"), project.getVersion());
			...
			if (project.getOrganization() != null) {
				...
				writer.println(
					"!define PROJECT_REG_KEY \"SOFTWARE\\{0}\\{1}\\{2}\"",
					new Object[] {project.getOrganization().getName(), project.getName(), project.getVersion()}
				);
				writer.println(
					"!define PROJECT_REG_UNINSTALL_KEY \"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{0} {1}\"",
					project.getName(),
					project.getVersion()
				);
				writer.println(
					"!define PROJECT_STARTMENU_FOLDER \"{0}\\{1}\\{2} {3}\"",
					new Object[] {"$SMPROGRAMS", project.getOrganization().getName(), project.getName(), project.getVersion()}
				);
			} else {
